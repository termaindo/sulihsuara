...
Gunakan bahasa yang inspiratif. Hindari kata-kata membosankan. Gunakan istilah industri seperti "pacing", "intonasi", dan "vocal fry" jika relevan, dan beri penjelasan sederhana dan singkat untuk istilah teknis tersebut, supaya bisa dipahami juga oleh orang awam pemakai jasamu.
    ...
    # --- 5. LOGIKA CHAT AI ---
    # Memori v6 untuk lembaran baru
    if "chat_naskah_v6" not in st.session_state:
        # KUNCI PERBAIKAN: Menggunakan mesin generasi baru karena 1.5 sudah dipensiunkan
        model_direktur = genai.GenerativeModel(
            model_name="gemini-2.5-flash", 
            system_instruction=DIREKTUR_PROMPT
        )
        st.session_state.chat_naskah_v6 = model_direktur.start_chat(history=[])
        
        try:
            st.session_state.chat_naskah_v6.send_message("Halo Direktur, saya siap membuat naskah baru. Tolong mulai tahap wawancaranya.")
        except Exception as e:
            st.error(f"Gagal menghubungi server Gemini API. Detail: {e}")
            st.stop()

    # Menampilkan riwayat chat ke layar (kecuali pesan pancingan awal)
    if len(st.session_state.chat_naskah_v6.history) > 1:
        for message in st.session_state.chat_naskah_v6.history[1:]:
        ...
        with st.chat_message("assistant"):
            try:
                with st.spinner("Direktur sedang menyusun strategi..."):
                    response = st.session_state.chat_naskah_v6.send_message(prompt_user)
